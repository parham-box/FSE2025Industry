...
def load_multi_vector_retriever(k,sl=None):
  ...
  return {"name": "test_documents", "retriever": reordered_retriever, "default":"Yes"}
def make_text(words):
...
def get_retrievers(type, chunk_size=0, child_chunk_size=0, chunk_overlap=0,child_chunk_overlap=0,k=3, extra_context_overlap=0):
  ...
  if load_presist == 'n':
    ...
    print("Function: get_retriever \t Status: Completed")  
    return [load_multi_vector_retriever(k=2,sl=dt['document_location']) for dt in documen_corpus_type]